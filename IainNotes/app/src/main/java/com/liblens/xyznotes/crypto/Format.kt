...
internal object Format {
    ...
    /** 3 — blob AAD now covers the header prefix, not just the path.
     *  Version 2 blobs do not authenticate their own AEAD id byte and are not
     *  readable by this version. Pre-release, so no migration is provided. */
    const val VERSION = 3
    ...
    /** Bytes of the blob header covered by AAD: magic, version, AEAD id,
     *  reserved. The nonce at [8,32) is excluded — it is already bound as the
     *  AEAD's npub, so covering it twice buys nothing. */
    const val BLOB_AAD_PREFIX = 8
